        ...
        if($request_method === 'GET'){
            switch ($page) {
                case 'totalBalance':
                    ...
                    break;
                case 'getTotalBalanceIncomeAndSplitBalance':
                    $this->getTotalBalanceIncomeAndSplitBalance();
                    break;
                case 'transactions':
...
// PHUC CHIA %
    ...
    private function getTotalBalanceByDate($from, $to)
    {
        ...
        // Lấy tiền total balance gán vào thẻ
        DB::table(TABLE_PREFIX.TABLE_ACCOUNTS)
    ->where("user_id", "=", $AuthUser->get("id"))
    // ->where("id", "=", $account->get("id"))
    ->update(['balance' => (double) $balance]);
     ...
     */
     ...
     //   Chia %
     private function getTotalBalanceIncomeAndSplitBalance()
    {
        /**Step 1 */
        $AuthUser = $this->getVariable("AuthUser");
        $date_type = Input::get("date");
        ...
        if(!in_array($date_type, ["week", "month", "year"])){
            $this->resp->msg = __("Date type is invalid! Balance Split");
            $this->jsonecho();
        }
        ...
        try 
        {
            ...
            if($date_type == "week"){
                ...
                $this->resp->week = $this->getTotalIncomeAndSplitBalanceByDate($from, $to);
            }else if($date_type == "month"){
                $this->resp->month = $this->getTotalIncomeAndSplitBalanceByDate($from, $to);
            }else if($date_type == "year"){
                $this->resp->year = $this->getTotalIncomeAndSplitBalanceByDate($from, $to);
            }
            // $this->resp->msg = __("F@@@@@@@@@@@@@@");
            $this->resp->result = 1;
        } 
     ...
     // Chia % mỗi lọ
     private function getTotalIncomeAndSplitBalanceByDate($from, $to)
     {
         $AuthUser = $this->getVariable("AuthUser");
     
         // Bước 1: Lấy tổng số tiền thu nhập trong khoảng thời gian đã chỉ định
         $income = DB::table(TABLE_PREFIX.TABLE_TRANSACTIONS)
             ->select(DB::raw('SUM(amount) as total'))
             ->where("user_id", "=", $AuthUser->get("id"))
            // ->where("user_id", "=", "95")
             ...
             ->whereBetween("transactiondate", $from->format("Y-m-d"), $to->format("Y-m-d"))
             ->first();
         $totalIncome = (double) $income->total;
     
         // Bước 2: Chia ra 6 dòng từ bảng mp_categories.balance
         $categoryIds = range(71, 76); // Lấy id từ 71 đến 76
         $loanPercentages = [55, 10, 10, 10, 10, 5]; // % cho mỗi khoản chi tiêu
         $totalBalance = [];
